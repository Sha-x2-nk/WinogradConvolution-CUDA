...
float *convCPU(float *img, float *F, int padding);
...
int main()
{
    ...
    float *out_actual = convCPU(img, f, padding);
    // printf("\n ACTUAL OUT: ");
...
float *convCPU(float *img, float *F, int padding)
{
    ...
    if(padding == 0){
        ...
        for (int n = 0; n < N; ++n)
            for (int k = 0; k < K; ++k)
                for (int h = FILTER_RADIUS; h < H - FILTER_RADIUS; ++h)
                    for (int w = FILTER_RADIUS; w < W - FILTER_RADIUS; ++w)
                    {
                        float tmp = 0;
                        for (int c = 0; c < C; ++c)
                        ...
                        out[((n * K + k) * OUT_H + h - FILTER_RADIUS) * OUT_W + w - FILTER_RADIUS] = tmp;
                    }
    }
    else{
        #pragma omp parallel for collapse(4)
